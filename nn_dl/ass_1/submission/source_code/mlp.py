...
#gradient versions
"""
The class for each type of gradient has three methods cupdate,vupdate and wupdate.
This is done keeping in mind the generality and readability of code. The different gradient methods
require at max two quantities other than the weights/parameters themselves. So we update 
these two extra parameters every time for every gradient option. This does have extra
computations but increases the plug and play ability of the code and makes it even more 
intuitive.
"""
...
class momentum(object):
    ...
    @staticmethod
    def vupdate(w,dw,vw,cw,eta,n):
        return vw*0.5-(eta/n)*dw#the constant was set to 0.5 based on the recommendation from stanford lectures
    @staticmethod
    def wupdate(w,dw,vw,cw,eta,n):
        return w+vw
...
class rmsprop(object):
    @staticmethod
    def cupdate(w,dw,vw,cw,eta,n):
        return 0.9*cw+(1-0.9)*dw**2 #the constant was set to 0.9 based on the recommendation from stanford lectures
    @staticmethod
    def vupdate(w,dw,vw,cw,eta,n):
        return vw
    @staticmethod
...
class Network(object):
    ...
    def feedforward(self, a):
        ...
        for b, w in zip(self.biases, self.weights):
            z=np.dot(w, a)+b
            a = self.actfun.fn(z)
        sftmx=np.exp(z-np.max(z)) #np.max(z) is done to keep the values in check.
    ...
    def grad_check(self, training_data):
        ...
        for i in xrange(2):
            b_db,b_dw = self.backprop(data[i][0],data[i][1],training_data)
            backprop_db+=b_db
            backprop_dw+=b_dw
        h=math.pow(10,-2)

        estimate=np.zeros(self.weights[1].shape)
        for j in xrange(2):
            for i,value in np.ndenumerate(self.weights[1]):
                epsilon = np.zeros(self.weights[1].shape)
                epsilon[i]=h
                self.weights[1]+=epsilon
                cost1=self.cost_indvdl(data[j][0],data[j][1] )
                self.weights[1]=self.weights[1]-2*epsilon
                cost2=self.cost_indvdl(data[j][0],data[j][1] )
                estimate[i]=(cost1-cost2)/(2*h)
                self.weights[1]+=epsilon
        
        return (estimate,backprop_dw[1])
    ...
    def cost(self, data ):
        ...
        for x,y in data:
            ...
            cost+= (np.sum(np.nan_to_num(-y*np.log(a))))/len(data)
        return cost

    def cost_indvdl(self, x,y):
        cost = 0.0
        a=self.feedforward(x)
        cost= np.sum(np.nan_to_num(-y*np.log(a)))
        return cost
    ...
    Sources : Internet in general
              Michael Nielsen's http://neuralnetworksanddeeplearning.com/ 
